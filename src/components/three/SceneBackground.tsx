...
export const SceneBackground: React.FC = () => {
  ...
  const cameraZ = useTransform(scrollYProgress, [0, 0.5, 1], [15, 8, -10]);
  const sceneRotation = useTransform(scrollYProgress, [0, 1], [0, Math.PI * 0.5]);
  const planetOpacity = useTransform(scrollYProgress, [0, 0.4, 0.7, 1], [1, 1, 0.5, 0]);
  ...
  useEffect(() => {
    ...
    const createEarthTexture = () => {
      ...
      const drawLand = (x: number, y: number, w: number, h: number, complexity: number) => {
        ...
        for(let i = 0; i < complexity; i++) {
          ...
          const rw = Math.random() * (w / 3);
          const rh = Math.random() * (h / 3);
      ...
      // Approximate Earth landmass locations
      drawLand(canvas.width * 0.25, canvas.height * 0.4, 600, 400, 100); // N. America
      drawLand(canvas.width * 0.3, canvas.height * 0.7, 400, 500, 80);  // S. America
      drawLand(canvas.width * 0.55, canvas.height * 0.3, 800, 400, 150); // Eurasia
      drawLand(canvas.width * 0.5, canvas.height * 0.6, 500, 400, 100);  // Africa
      drawLand(canvas.width * 0.8, canvas.height * 0.7, 300, 250, 60);  // Australia

      // 3. Neural Network "City Lights"
    ...
    const fillLight = new THREE.PointLight("#C41BFD", 1.5, 100);
